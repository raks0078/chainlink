...
func (ekc *ETHKeysController) Index(c *gin.Context) {
	...
	var resources []webpresenters.ETHKeyResource
	for _, key := range keys {
		...
		r, err := webpresenters.NewETHKeyResource(k,
...
func (ekc *ETHKeysController) Create(c *gin.Context) {
	...
	r, err := webpresenters.NewETHKeyResource(key,
		ekc.setEthBalance(c.Request.Context(), key.Address.Address()),
		ekc.setLinkBalance(key.Address.Address()),
	)
	if err != nil {
...
// resource.
func (ekc *ETHKeysController) setEthBalance(ctx context.Context, accountAddr common.Address) webpresenters.NewETHKeyOption {
	store := ekc.App.GetStore()
	bal, err := store.EthClient.BalanceAt(ctx, accountAddr, nil)

	return func(r *webpresenters.ETHKeyResource) error {
		if err != nil {
...
// resource.
func (ekc *ETHKeysController) setLinkBalance(accountAddr common.Address) webpresenters.NewETHKeyOption {
	store := ekc.App.GetStore()
